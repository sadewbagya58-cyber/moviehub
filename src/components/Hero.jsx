...
// Fisher-Yates shuffle — returns a brand-new shuffled copy, never mutates the original.
...
const Hero = ({ movies = [] }) => {
  // shuffledMovies lives in local state so it is immune to Firebase re-renders.
  const [shuffledMovies, setShuffledMovies] = useState([]);
  const [currentIndex, setCurrentIndex] = useState(0);

  // Track whether we have already shuffled so Firestore's repeated snapshot
  // callbacks don't reset the order after the user has started watching.
  const hasShuffled = useRef(false);

  // Fire once – the first time real data arrives from Firebase (length > 0).
  useEffect(() => {
    if (movies.length > 0 && !hasShuffled.current) {
      hasShuffled.current = true;
      setShuffledMovies(shuffleArray(movies));
      setCurrentIndex(0); // start from the first item of the shuffled list
    }
  }, [movies.length]); // keyed on length so it re-fires if the catalog grows
                       // but hasShuffled.current prevents a second full re-shuffle.

  // Autoplay: advance sequentially through the already-shuffled list.
  // Depends on shuffledMovies so the interval always has a fresh reference.
  useEffect(() => {
    if (shuffledMovies.length <= 1) return;
  ...
  return (
    <section className="relative w-full h-[65vh] md:h-[85vh] overflow-hidden bg-brand-bg">
      {shuffledMovies.map((movie, index) => {
        ...
        const typePrefix =
          movie.type === 'TV Series' || movie.type === 'TV' ? 'tv' : 'movie';
        ...
        return (
          ...
          >
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
              <div
                className={`max-w-2xl space-y-4 sm:space-y-6 transition-all duration-1000 delay-300 ${
                  isActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                }`}
              >
                {/* Labels & Meta */}
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                  <span className="bg-brand-accent/20 text-brand-accent border border-brand-accent/30 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold tracking-wider uppercase">
                    Trending{' '}
                    {movie.type === 'TV Series' || movie.type === 'TV'
                      ? 'Show'
                      : 'Movie'}
                  </span>
                  <div className="flex items-center gap-1.5 sm:gap-2 text-yellow-400 font-black text-sm sm:text-lg">
                    <Star
                      size={16}
                      fill="currentColor"
                      className="sm:w-5 sm:h-5 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]"
                    />
